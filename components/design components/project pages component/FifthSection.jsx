...
export function FifthSection({Title1, text, Title2, list2}) {
  ...
  const items = [
    ...
    return (
        (<BentoGrid className="mx-auto md:grid-cols-3 max-w-8xl gap-20">
          {items.map((item, i) => (
            <BentoGridItem
              key={i}
              title={item.title}
              ...
              className={i === 0 ? "md:col-span-2 md:col-start-1 ml-4" : i === 1 ? "md:col-span-2 md:col-start-2 items-end mr-4" : ""} />
          ))}
        </BentoGrid>)
    );
}
